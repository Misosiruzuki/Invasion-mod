...
public class Wave {
    ...
    public void addEntry(WaveEntry entry) { entries.add(entry); }
    ...
    public int getTimeInWave() { return elapsed; }
    public int getWaveTotalTime() { return waveTotalTime; }
    public int getWaveBreakTime() { return waveBreakTime; }
    public boolean isComplete() { return elapsed > waveTotalTime; }

    public void resetWave() {
        elapsed = 0;
        for (WaveEntry entry : entries) entry.resetToBeginning();
    }

    public void setWaveToTime(int millis) { elapsed = millis; }

    public int getTotalMobAmount() {
        int total = 0;
        for (WaveEntry entry : entries) total += entry.getAmount();
        return total;
    }
}
